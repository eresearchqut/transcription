...
export const TranscriptionScript = (props) => {
    const speakerSegments = props.speakerLabels.segments.map((segment, index) => [segment, props.scriptSegments[index]]);
    ...
    const dialogueSegments = speakerSegments.map((segment, index) => {
        ...
        return <DialogueSegment key={index} mainSpeaker={mainSpeaker}>
            {transcript}
        </DialogueSegment>
    })
