...
export class GitHubStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: GitHubStackProps) {
    ...
    const principal = new iam.WebIdentityPrincipal(
      ...
      {
        StringLike: {
          "token.actions.githubusercontent.com:sub": filters.map(
            (filter) => `repo:${props.owner}/${props.repo}:${filter}`,
          ),
        },
      },
    );
    ...
    const cloudformationPolicy = new iam.PolicyStatement({
      ...
      resources: props.stacks.map(
        (stack) =>
          `arn:aws:cloudformation:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:stack/${stack}/*`,
      ),
    });
    ...
    const ssmPolicy = new iam.PolicyStatement({
      ...
      resources: [
        `arn:aws:ssm:*:${cdk.Aws.ACCOUNT_ID}:parameter/app/${props.envName}/${props.repo}/env`,
      ],
    });
    ...
    new cdk.CfnOutput(this, "deployRoleArn", {
      ...
      description:
        "The role used via GitHub Actions to deploy with AWS CDK on the target AWS account",
