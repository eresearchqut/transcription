...
`config/local.json` writes the browser-facing host as `${LOCAL_HOST}`, which `bin/deployment.ts` substitutes from the environment variable of that name, defaulting to `localhost`. It covers the emulator endpoint and the frontend origin, both of which the browser resolves, so opening the app from another device needs this machine's reachable address rather than `localhost`. The endpoint the deploy and the handlers use is unrelated and stays on the Compose network.
