 ...
 *
 * `custom:eResearchGroups` is not seeded. LocalUserPoolStack declares the
 * attribute so the local pool matches the real schema, but the value is set by
 * the identity provider on the externally managed pool and its format is not
 * known here.
 *
 * Usage:
 *   pnpm ministack:up
